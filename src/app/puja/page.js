...
export default function PujaPage() {
  ...
  const pujaTypes = ["Marriage",
    "Birth Related ",
    "Death Rituals (Ancestoral Rites)",
    "Festivities",
    "Ceremonial",
    "Business",
    "Path [Recitation]",
  ...
  return (
    <div className="bg-[#e5e5e5]">
      <div className="flex justify-around items-center">
        ...
        <div>
          <input type="text" placeholder="Search" alt="search bar" className="border-[1px] border-gray-400 h-10 px-6 py-4 min-w-72 rounded-md transition duration-300 focus:outline-none focus:ring-2 focus:ring-orange-600 focus:border-orange-600 focus:shadow-lg" />
      ...
      <div className="grid grid-cols-12 gap-16 px-32 items-start">
        <div className="col-span-3 py-6 px-4 bg-white border-t-8 border-orange-600 flex gap-2">
          ...
          <div className="flex flex-col items-start w-full">
            <span className="text-white text-sm font-semibold w-full rounded-md bg-orange-600 p-2 mb-6 hover:cursor-pointer">PUJA SERVICES</span>
            {
              pujaTypes.map((onetype) => {
                return (
                  <button className="my-1 p-2 text-left w-full rounded-md font-semibold hover:bg-gray-200 hover:border-[0.5px] border-orange-600 hover:text-orange-600" onClick={() => handleTypeChange(onetype)}>{onetype}</button>
                )
              })
            }
          </div>
